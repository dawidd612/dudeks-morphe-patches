...
Select Piko **Add settings** in the same patching run, then enable **Piko Settings > Direct Messages > Keep scroll position when replying**.
The option is disabled by default. See [implementation, compatibility and phone testing](docs/instagram-dm-scroll.md).
The release index above is generated by the release workflow; this feature branch is not a published release.
